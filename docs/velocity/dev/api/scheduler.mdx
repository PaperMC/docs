...
All scheduling works by using a <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/scheduler/Scheduler.TaskBuilder.html"}>TaskBuilder</VersionedJavaDocLink> returned from the [Scheduler](https://jd.papermc.io/velocity/3.0.0/com/velocitypowered/api/scheduler/Scheduler.html). This fluent builder may
...
You can also use a [java.time.Duration](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/time/Duration.html) to specify the time arguments, e.g.: `Duration.ofSeconds(5L)`.
...
The `schedule()` method returns a <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/scheduler/ScheduledTask.html"}>ScheduledTask</VersionedJavaDocLink>, which may then be used to cancel the task
