...
Listening to events with Velocity's <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/Subscribe.html"}>`@Subscribe`</VersionedJavaDocLink> annotation is straightforward. You've already seen
...
To listen to an event, mark the method with <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/Subscribe.html"}>`@Subscribe`</VersionedJavaDocLink>, like shown. This works similarly to
...
As an alternative to `@Subscribe`, you can also use the functional <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/EventHandler.html"}>`@EventHandler`</VersionedJavaDocLink> interface and
...
return an <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/EventTask.html"}>`EventTask`</VersionedJavaDocLink> or add a second <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/Continuation.html"}>`Continuation`</VersionedJavaDocLink> parameter:
...
A functional listener simply needs to implement <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/AwaitingEventExecutor.html"}>`AwaitingEventExecutor`</VersionedJavaDocLink> and return an <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/EventTask.html"}>`EventTask`</VersionedJavaDocLink>:
...
that this returns a [`CompletableFuture`](https://docs.oracle.com/en/java/javase/21/docs/api/java.base/java/util/concurrent/CompletableFuture.html), so if you want to continue logic after the event is handled
...
Velocity uses the generalised <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/ResultedEvent.html"}>`ResultedEvent`</VersionedJavaDocLink> for events which have some sort of 'result'. The
...
Some common result types are <VersionedJavaDocLink project={"velocity"} target={"/com/velocitypowered/api/event/GenericResult.html"}>`GenericResult`</VersionedJavaDocLink>, for simple allowed/denied results, and component
