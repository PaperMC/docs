---
slug: /dev/command-api/arguments/bukkit-arguments
description: Documentation for the BlockState, ItemStack, and a few more commonly used Bukkit related values
---
...
## ItemStack Argument
The item stack argument is a way to retrieve an <JavaDocs name={org.bukkit.inventory.ItemStack}>`ItemStack`</JavaDocs> following the same argument format as the vanilla `/give <player> <item> [<amount>]`
command as its second argument. The user may also define components to further customize the `ItemStack`. If you only require a <JavaDocs name={org.bukkit.Material}>`Material`</JavaDocs>, you should instead
...
<JavaDocs name={org.bukkit.NamespacedKey}>`NamespacedKey`</JavaDocs>, which makes it particularly useful when dealing with Bukkit API.
