...
import SignedMessagesDeletionMp4 from "./assets/signed-messages-deletion.mp4";
...
messages it sends in order to be uniquely identifiable and verifiable to be sent by a specific player.
...
object to represent a signed message. We differentiate two kinds of signed message: system messages and non-system messages.
...
You can also differentiate the **signed plaintext** `String` content of the message
...
### In-game preview
<Video src={SignedMessagesDeletionMp4} play-always />

### Code
```java title="SignedChatListener.java" collapse={1-11} showLineNumbers
