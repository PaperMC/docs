...
- If `count = 0`, the client uses the provided location without modification. The offset values are multiplied by the
`extra` argument and passed to the particle constructor. The way these values are
used may vary between particle types.

- If `count > 0`, the client spawns that number of particles. For each particle, it generates random offset
