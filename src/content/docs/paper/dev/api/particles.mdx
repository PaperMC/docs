...
- If `count > 0`, the client spawns `count` number of particles. For each particle, it generates random offset
...
- If set to `0`, the `offsetX` argument will be used in a custom function to determine the color of the note particle,
...
- If set to a positive number, offset arguments are used normally and a random target location is picked.
...
a y-axis velocity based on `offsetY`.

In both cases, `offsetX` and `offsetZ` are not used in the velocity vector.
