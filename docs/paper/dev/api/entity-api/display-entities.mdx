...
Displays can also be added as passengers to entities with the
...
If you plan to use the display only temporarily, make sure to remove it with
<Javadoc name={"org.bukkit.entity.Entity#remove()"}>`Entity#remove()`</Javadoc> afterward.
...
player.addPassenger(display); // mount it on top of the player's head
player.showEntity(plugin, display); // show it to the player
// ...
