...
:::note
The use of [`ParticleBuilder.receivers(48, true)`](jd:paper:com.destroystokyo.paper.ParticleBuilder#receivers(int,boolean))
select all players in a distance of 48 blocks from the particle's location,
...
In the following example 8 `FLAME` particles are spawned in a 1x1x1 cube shape randomly. `someLocation` servers as it's center.
...
<Tabs syncKey="spawn-type">
  <TabItem label="ParticleBuilder">
    ...
    ParticleBuilder particleBuilder = Particle.DUST.builder()
      .color(Color.GREEN, 2.0f);

    // We can reuse the builder
    for (double i = 0; i <= 1.0; i += 0.1) {
      particleBuilder.location(someLocation.clone().add(0, i, 0)).receivers(48, true).spawn();
    }
    ```
  </TabItem>
  <TabItem label="spawnParticle">
    ...
    for (double i = 0; i <= 1.0; i += 0.1) {
      someWorld.spawnParticle(
        ...
        1,
        new Particle.DustOptions(Color.GREEN, 2.0f)
      );
    }
    ```
  </TabItem>
...
from one to another. A [Particle.DustTransition](jd:paper:org.bukkit.Particle$DustTransition) is used for specifying the
transition.
...
(`someLocation.clone().add(12, 0, 0)`) with a red color and a travel time of 10 ticks:
<Tabs syncKey="spawn-type">
  <TabItem label="ParticleBuilder">
    ```java
    Particle.TRAIL.builder()
      ...
      .offset(2, 2, 2)
      .count(8)
      .data(new Particle.Trail(someLocation.clone().add(12, 0, 0), Color.RED, 10))
      .receivers(48, true)
      .spawn();
    ```
  </TabItem>
  <TabItem label="spawnParticle">
    ...
    someWorld.spawnParticle(
      ...
      8,
      2, 2, 2,
      new Particle.Trail(someLocation.clone().add(12, 0, 0), Color.RED, 10)
    );
    ```
  </TabItem>
</Tabs>
...
### Explosion particles
`EXPLOSION` and `SONIC_BOOM`'s scale is calculated as `2.0 * (1.0 - offsetX * 0.5)`.
...
location. When they reach the player's y level, they will stop moving vertically.

If the player is moving vertically, the particles will attempt to match they player's vertical velocity.

### Splash particles
The `SPLASH` particle adds the `offsetX` and `offsetZ` values to the particle's velocity vector's x and y components, respectively,
if `offsetY` is `0` and `offset` or `offsetZ` are not `0`.

### Damage indicator particles
The `DAMAGE_INDICATOR` particle increases the provided `offsetY` by `1.0`. To make the velocity vector have zero,
set `offsetY` to `-1.0`.
...
The `DUST_PLUME` particle increases the provided `offsetY` by `0.15`. To make the velocity vector have zero,
set `offsetY` to `-0.15`.
