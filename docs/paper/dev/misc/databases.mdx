...
and the main two are SQL and NoSQL.

### NoSQL vs SQL

A NoSQL (Not Only SQL) database is a type of database management system that diverges from the traditional relational database model. 
...
and are usually used for smaller databases. Standalone databases are stored in a separate process, and are usually used for larger databases.
...
To learn more about the Java Database Connection, see [here](https://www.baeldung.com/java-jdbc)

</details>

### Standalone Databases

Standalone databases are stored in a separate process, and are usually used for larger databases. They are harder to set up and use,
...
The connectors for these databases often have connection pooling. Database connection pooling addresses this issue by
creating a pool of pre-established and reusable database connections. Instead of opening a new connection every time a
