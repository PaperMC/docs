...
check out the [Extensive Guide on Command Execution](./command-execution) after you finish reading this.
...
| StringArgumentType.greedyString() | String        | unqouted spaces   | The literal written input. May contain any characters. Has to be the last argument |
...
you can enter any combination of unicode characters you want to. Special characters, like quotes `"`, can be escaped using a backspace `\`.
* ✅ `this_is_valid_input`
* ❌ `this is invalid input`
...
## Further references
It is suggested that you visit our [Extensive Guide on Command Execution](./command-execution) in order to read more about the `.executes(...)` method. It contains a lot of useful information
