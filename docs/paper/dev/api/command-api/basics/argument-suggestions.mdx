...
- Using the `MessageComponentSerializer`, with which you can serialize `Component`s into `Message`s.

For example, if you add a suggestion like this:
```java
builder.suggest("suggestion", MessageComponentSerializer.message().serialize(miniMessage().deserialize("<green>Suggestion tooltip")));
```
