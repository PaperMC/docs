...
The first option is using the [](jd:paper:com.destroystokyo.paper.ParticleBuilder) class, which is
...
## Colored particles
These particles can be colored by passing a [](jd:paper:org.bukkit.Color) object as the `data` argument.
...
[](jd:paper:org.bukkit.Particle$DustOptions) as `data`.
...
### Dust transition
Dust transition particles work exactly like [dust particles](#dust), but instead of having a static color, they **transition**
their color from one to another. A [](jd:paper:org.bukkit.Particle$DustTransition) is used for
...
### Trail
Trail particles require you to pass a [](jd:paper:org.bukkit.Particle$Trail) object as `data`.
...
:::note
This guide uses [](jd:paper:org.bukkit.block.BlockType#createBlockData()). While using
[](jd:paper:org.bukkit.Material#createBlockData()) or
[](jd:paper:org.bukkit.Bukkit#createBlockData(org.bukkit.Material)) yields the same result,
...
:::note
This guide uses [](jd:paper:org.bukkit.inventory.ItemStack#of(org.bukkit.Material)). While using
[`new ItemStack(Material)`](jd:paper:org.bukkit.inventory.ItemStack) yields the same result,
it is considered **legacy**. [](jd:paper:org.bukkit.inventory.ItemType#createItemStack())
...
### Vibration
Vibration particles require you to pass a [](jd:paper:org.bukkit.Vibration) object as `data`, where you can choose between a
location ([](jd:paper:org.bukkit.Vibration$Destination$BlockDestination))
or an entity target ([](jd:paper:org.bukkit.Vibration$Destination$EntityDestination)).
...
:::caution
`EFFECT` and `INSTANT_EFFECT` are [powered particles](#powered-particles) and use [](jd:paper:org.bukkit.Particle$Spell)
...
those particles are covered in [Dust particles](#dust) and [Dust transition particles](#dust-transition) chapters.
...
[](jd:paper:org.bukkit.Particle$GeyserBase) as their data.

The particle size is calculated as `3.0 + 0.125 * waterBlocks`.
...
### Plume
The `GEYSER_PLUME` particle uses [](jd:paper:org.bukkit.Particle$Geyser) as its data.
...
This particle uses [](jd:paper:org.bukkit.Particle$Geyser) as its data.
