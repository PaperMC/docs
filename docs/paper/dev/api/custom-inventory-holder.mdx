...
<VersionedJavaDocLink target={"/org/bukkit/inventory/InventoryHolder.html"}>InventoryHolder</VersionedJavaDocLink> is an interface that we must implement.
We can do this the following way: create a new class that will create our <VersionedJavaDocLink target={"/org/bukkit/inventory/Inventory.html"}>`Inventory`</VersionedJavaDocLink> in the constructor.
