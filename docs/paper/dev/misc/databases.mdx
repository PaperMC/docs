...
You must invoke a `Class.forName` on the driver to allow it to initialize and then create the connection to the database:
...
Using prepared statements in Java with `PreparedStatement`s helps prevent SQL injection.
