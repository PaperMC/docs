...
Every <Javadoc name={"org.bukkit.event.Event"}>`Event`</Javadoc> that can be listened to has a
<Javadoc name={"org.bukkit.event.HandlerList"}>`HandlerList`</Javadoc> containing all the listeners that are listening to that event.
...
You can unregister based on <Javadoc name={"org.bukkit.event.Listener"}>`Listener`</Javadoc>
or <Javadoc name={"org.bukkit.plugin.Plugin"}>`Plugin`</Javadoc> for more convenience.
Likewise, you can also unregister all listeners for a specific event by calling
<Javadoc name={"org.bukkit.event.HandlerList#unregisterAll()"}>`unregisterAll()`</Javadoc> on the `HandlerList`.
