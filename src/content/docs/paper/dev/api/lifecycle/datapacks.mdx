...
requires you to use a [Paper plugin](/paper/dev/getting-started/paper-plugins) (with a `paper-plugin.yml`).
...
The following code examples are assumed to be executed inside a `PluginBootstrap`'s `bootstrap` method.
They are also assumed to already be inside an event handler, resulting in a structure like this:

```java title="ExamplePluginBootstrap.java" showLineNumbers
...
```
[20:36:22 INFO]: [ExamplePlugin] The following datapacks were found: minecart_improvements, paper, redstone_experiments, trade_rebalance, vanilla
```
...
You can very easily prevent datapacks from being discovered by calling [](jd:paper:io.papermc.paper.datapack.DatapackRegistrar#removeDiscoveredPack(java.lang.String))
with the datapack's name.

To remove the above-mentioned, experimental datapacks, you could use the following code:
...
```
[20:43:42 INFO]: [ExamplePlugin] The following datapacks were found: paper, vanilla
```

## Registering custom datapacks
The main use case of the datapack discovery lifecycle event is including datapacks from plugins. And Paper makes this pretty simple:
...
This means including the **source files** directly, which makes it very convenient to work with in a dev environment.
...
you can check out [Vanilla Tweaks](https://vanillatweaks.net/picker/datapacks) for their data packs.

:::

### Including the datapack in your plugin
Before you can let the server know about your datapack, you must first include it. For this, you can just add it to your plugin's resources source set (usually located in **src/main/resources**).
The datapack should have at least one extra folder (so don't dump all the contents into the **resources** folder). For this example, it will be located under
...
To discover the datapack, you must call the [](jd:paper:io.papermc.paper.datapack.DatapackRegistrar#discoverPack(java.net.URI,java.lang.String))
method. The URI should point to **your datapack's folder in your JAR**. This can be achieved simply by calling
`this.getClass().getResource("/custom_datapack").toURI()`. **The preceding slash is very important**. The id can be set to
...
try {
    ...
    final URI uri = this.getClass().getResource("/custom_datapack").toURI();
...
```java title="ExamplePlugin.java" showLineNumbers
public final class ExamplePlugin extends JavaPlugin {
    ...
    public void onLoad() {
        ...
        if (pack != null && pack.isEnabled()) {
            this.getSLF4JLogger().info("The datapack loaded successfully!");
        } else {
            this.getSLF4JLogger().warn("The datapack failed to load.");
        }
    }
}
```
...
```
[20:51:25 INFO]: [ExamplePlugin] Loading server plugin ExamplePlugin v1.0
[20:51:25 INFO]: [ExamplePlugin] The datapack loaded successfully!
```
