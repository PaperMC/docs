...
## Resource argument
Just like any other argument, you can get an `ArgumentType<T>` reference to it using `ArgumentTypes.resource(RegistryKey<T>)`.
They are accessed in a static context using the [](jd:paper:io.papermc.paper.registry.RegistryKey) interface.
...
Commands.argument("enchantment", ArgumentTypes.resource(RegistryKey.ENCHANTMENT))
    .executes(context -> {
        ...
        if (item.isEmpty()) {
            context.getSource().getSender().sendRichMessage("<red><player> is not holding any item",
        ...
        item.addUnsafeEnchantment(enchantment, 10);
        context.getSource().getSender().sendRichMessage("Enchanted <player>'s <item> with <enchantment>!",
            Placeholder.component("player", player.displayName()),
...
We define an `enchantment` argument using the registry key of the enchantment registry and retrieve the value of that using `context.getArgument("enchantment", Enchantment.class)`.
...
For the client, there is barely any difference between using `ArgumentTypes.resource` or `ArgumentTypes.resourceKey`. The only difference is that
...
In the example given above, due to an unhandled `NullPointerException`, the command does not successfully run. The code for that command is directly trying to use the value retrieved
by the registry access by doing `RegistryAccess.registryAccess().getRegistry(itemKey.registryKey()).get(itemKey.key())`. If you try to do any
operation with the result, it might be null and error.

You should **always** check the result of a registry retrieval operation when it's not already enforced by the argument type. An example for that is given below in the [direct code comparison](#direct-code-comparison).
...
Here is a simple code snippet on how one could use the `RegistryKey.ITEM` registry with a `resource` argument type:

```java
Commands.argument("item", ArgumentTypes.resource(RegistryKey.ITEM))
    .executes(context -> {
        final ItemType item = context.getArgument("item", ItemType.class);
        final Player player = context.getSource().getPlayerOrThrow();
...
Here is the same code, using a `resourceKey` argument type. Instead of directly retrieving the argument using `context.getArgument("item", TypedKey.class)`, we instead use the
[](jd:paper:io.papermc.paper.command.brigadier.argument.RegistryArgumentExtractor) to retrieve our `TypedKey<ItemType>`.

```java
Commands.argument("item", ArgumentTypes.resourceKey(RegistryKey.ITEM))
    .executes(context -> {
        final TypedKey<ItemType> itemKey = RegistryArgumentExtractor.getTypedKey(context, RegistryKey.ITEM, "item");
        final ItemType item = RegistryAccess.registryAccess().getRegistry(itemKey.registryKey()).get(itemKey.key());

        if (item == null) {
            context.getSource().getSender().sendRichMessage("<red>Please provide a valid item!");
            return Command.SINGLE_SUCCESS;
        }

        context.getSource().getPlayerOrThrow().getInventory().addItem(item.createItemStack());
        return Command.SINGLE_SUCCESS;
    })
```
