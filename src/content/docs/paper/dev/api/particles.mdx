...
- If set to `0`, the offset arguments will be used for a fixed relative spawn location from the supplied location/coordinates.
...
:::note
This guide uses [`ItemType.createItemStack()`](jd:paper:org.bukkit.inventory.ItemType#createItemStack()). While using
[`new ItemStack(Material)`](jd:paper:org.bukkit.inventory.ItemStack) yields the same result,
it is considered **legacy**.
:::
...
### Vibration
Vibration particles are really simple to spawn. Pass a [`Vibration`](jd:paper:org.bukkit.Vibration) object as `data`, where you can choose between a
