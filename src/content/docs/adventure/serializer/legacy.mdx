...
using `/minimessage/index`.

<AdventureDependency artifact="adventure-text-serializer-legacy" project="api" />
...
LegacyComponentSerializer.legacyAmpersand(); // For '&'
LegacyComponentSerializer.legacySection(); // For '§'
LegacyComponentSerializer.legacy(char legacyCharacter); // For any symbol
```
