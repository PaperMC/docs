...
This means that all the [](jd:adventure:net.kyori.adventure.audience.Audience)
methods are available on [](jd:paper:org.bukkit.command.CommandSender),
[](jd:paper:org.bukkit.Server), [](jd:paper:org.bukkit.scoreboard.Team)
and [](jd:paper:org.bukkit.World).

## `ForwardingAudience`

The [](jd:adventure:net.kyori.adventure.audience.ForwardingAudience)
wraps a collection of [](jd:adventure:net.kyori.adventure.audience.Audience)
...
They are mostly used for sending other parts of the API to players. For example, you can send a [](jd:adventure:net.kyori.adventure.text.Component)
to a player using [](jd:adventure:net.kyori.adventure.audience.Audience#sendMessage(net.kyori.adventure.text.Component)).
...
```java
// Get the uuid from an audience member, returning a UUID if available
Optional<UUID> uuid = audience.get(Identity.UUID);

// Get the display name, returning a default value if not available
