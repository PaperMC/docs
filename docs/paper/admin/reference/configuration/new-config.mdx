...
chunk-loading:
  autoconfig-send-distance:
    default: true
    description: |
      Whether to use the [client's](https://www.google.com) view distance for the **chunk** __send__ *distance* of the server.
      This will exclusively change the radius of chunks sent to the client and will not affect server-side chunk loading or ticking.
