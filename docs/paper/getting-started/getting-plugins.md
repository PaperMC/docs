...
this, it is imperative that plugins be installed only from trusted sources. Be careful!

:::

## Finding Plugins

Before you can install a plugin, you'll need to find what you want to install. Most plugins can be
found on [SpigotMC](https://www.spigotmc.org/resources/),
...
you desire followed by `Minecraft plugin` will often yield good results.

:::tip Spigot and Bukkit Plugins

Paper is compatible with both Spigot and Bukkit plugins. It's okay if a plugin does not specifically
mention Paper. It'll still work.
...
servers most recent logs will be stored to the `logs/latest.log` file. You may need to scroll near
