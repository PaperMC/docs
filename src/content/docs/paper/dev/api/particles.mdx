...
The `SWEEP_ATTACK` particle has two different behaviors based on the `count` argument:
- If set to a positive number, offset arguments are used normally and a random target location.
- If set to `0`, the `offsetX` argument controls the particle's scale, while `offsetY` and `offsetZ` are ignored.
...
:::note
The scale is calculated as `1.0 - offsetX * 0.5`.
:::
...
### List of y-axis velocity particles
<details>
  <summary>Show list</summary>
  - EFFECT
  - ENTITY_EFFECT
...
### Explosion particles
The `EXPLOSION` particle uses the `offsetX` argument as the explosion's size. It is calculated as `2.0 * (1.0 - offsetX * 0.5)`.
Negative final values will result in the particle appearing mirrored.
...
`extra` argument and passed to the particle constructor. The way these values are
