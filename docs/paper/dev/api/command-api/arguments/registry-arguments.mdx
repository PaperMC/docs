...
<FullWidthVideo src={ResourceKeyItem}/>
<br/>
...
Unless you have a specific reason for using the `resourceKey` argument over the `resource` one, the `resource` argument is prefered due to the error checking
and simple usability.

:::

### Direct code comparison
Here is a simple code snipped on how one could use the `RegsitryKey.ITEM` registry with a `resource` argument type:
