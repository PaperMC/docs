...
Plugin messages are formatted as byte arrays and can be sent using the <Javadoc name={"org.bukkit.plugin.messaging.PluginMessageRecipient#sendPluginMessage(org.bukkit.plugin.Plugin,java.lang.String,byte[])"}>`sendPluginMessage`</Javadoc>
method on a <Javadoc name={"org.bukkit.entity.Player"}>`Player`</Javadoc> object.
...
Our proxy server received the message through the player who triggered the <Javadoc name={"com.destroystokyo.paper.event.player.PlayerJumpEvent"}>`PlayerJumpEvent`</Javadoc> on our Java server.
