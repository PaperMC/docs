...
To create a custom event, you need to create a class that extends <VersionedJavaDocLink target={"/org/bukkit/event/Event.html"}>`Event`</VersionedJavaDocLink>. Each event requires a <VersionedJavaDocLink target={"/org/bukkit/event/HandlerList.html"}>`HandlerList`</VersionedJavaDocLink> that will contain all the listeners that are listening to that event. The only exception to this requirement is when you have an event class that cannot be fired, but serves as a parent for other events instead.
...
If you want to allow your event to be cancelled, you can implement the <VersionedJavaDocLink target={"/org/bukkit/event/Cancellable.html"}>`Cancellable`</VersionedJavaDocLink> interface.
