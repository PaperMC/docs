...
Commands are registered in the same way in a
<Javadoc name={"io.papermc.paper.plugin.bootstrap.PluginBootstrap#bootstrap(io.papermc.paper.plugin.bootstrap.BootstrapContext)"}>plugin's bootstrapper</Javadoc>.
The benefit of registering commands here is that they will be available to datapack functions because
the command registration happens early enough.
