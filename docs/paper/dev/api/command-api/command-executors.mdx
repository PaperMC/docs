---
slug: /dev/command-api/command-executors
description: A guide to execution logic for Brigadier commands
---

import FlySpeedPlayer from "./assets/in-game/flyspeed-player.png"
import FlySpeedConsole from "./assets/in-game/flyspeed-console.png"
import FlySpeedProxied from "./assets/in-game/flyspeed-proxied.png"

# Command Executors

In the previous chapters we have taken a look at the general Brigadier structure and the way to define arguments using Brigadier. If you haven't read through those articles, I highly
recommend visiting the page about the [Command Tree](./command-tree), and [Arguments and Literals](./arguments-and-literals).
...
There is a lot to unpack about this, so let's break it down, top to bottom:
...
## Logic separation
Sometimes, if the command is big enough, or due to personal preference, you might not want to have your logic code in your executes method, as it might be unreadable
because of the amount of indentations. In such an event, we can, instead of defining our logic in the lambda statement, use a method reference instead. For that we
define a method in the same context and class as our command declaration. This might look like this:
```java title="FlightSpeedCommand.java"
