...
### Emitter
The `GEYSER` particle is an emitter particle, which means that it spawns other particles. In this case, it spawns all the
above-mentioned geyser particles. The `offsetX` and `offsetZ` arguments are ignored, while `offsetY` is used to determine the vertical velocity of the spawned particles.
