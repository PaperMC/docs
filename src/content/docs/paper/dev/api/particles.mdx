...
which selects players that have a distance of up to 48 blocks (3 chunks) from the particle's location. Using `receivers(48, false)` or `receivers(48)`
would select players that are within a 96x96x96 cuboid centered around the particle's location.
:::
