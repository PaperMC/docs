...
To listen for events, you need to create a class that implements <VersionedJavaDocLink target={"/org/bukkit/event/Listener.html"}>`Listener`</VersionedJavaDocLink>.
...
To listen for an event, you need to create a method that is annotated with <VersionedJavaDocLink target={"/org/bukkit/event/EventHandler.html"}>`@EventHandler`</VersionedJavaDocLink>.
