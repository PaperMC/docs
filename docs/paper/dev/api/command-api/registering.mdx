---
slug: /dev/command-api/registering
description: A guide to registering Brigadier commands
---

# Command Registration
In the previous chapters, we have taken an extensive look at how Brigadier works, but never actually specified on how to register commands. So we will be doing that right here!
...
### Using the PluginBootstrap
:::note

This requires you to use a `paper-plugin.yml` plugin. You can read more on that
[here](../getting-started/paper-plugins). If you are not using `paper-plugin.yml`, you can instead register your commands inside your plugin's main class.
...
The rest of the methods works the exact same way as with the `PluginBootstrap` inherited class.
...
method. We also have access to the `CommandDispatcher<CommandSourceStack>` via `Commands#getDispatcher()`, which is a way more advanced access to registering commands, and
the `Commands#registerWithFlags()` method, which is also irrelevant for basic usage. We will only be looking at the overloads of the `Commands#register` method for now.
...
Similar to the `LiteralCommandNode`, we also have overloads for setting various additional information for our command. You can check out the **JavaDocs** for more information.

## Further references
* For a quick reference on the LifecycleEventManager, you can use **this site**.
* If you want to read more about lifecycle events, you can check out the [dedicated docs entry](../lifecycle.mdx)