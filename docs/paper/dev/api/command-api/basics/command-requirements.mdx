---
slug: /dev/command-api/basics/command-requirements
...
You don't have to limit yourself to checking just for permissions - any boolean can be returned, since it is a predicate. For example, you can check for whether
...
How can we solve this? The `Player` interface has a method called `#updateCommands()` just for this usecase. It resends the currently registered commands back to the
...
Forcing a player to reload their own commands is not a viable option for user experience. For this reason, you can **automate** this behaviour. It is safe to call
