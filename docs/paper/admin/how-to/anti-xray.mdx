...
[Per-World Configuration Reference](../reference/configuration/world-configuration.mdx#anticheat_anti_xray).
...
with randomly generated `hidden-blocks`. `engine-mode: 3` works the similarly to `engine-mode: 2`, but instead of randomizing every block, it only randomizes blocks per chunk layer.
...
[`feature-seeds` configuration](../reference/configuration/world-configuration.mdx#feature_seeds), in conjunction
