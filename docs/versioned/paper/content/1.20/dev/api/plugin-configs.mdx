...
The <Javadoc name={"org.bukkit.configuration.file.FileConfiguration"}>`FileConfiguration`</Javadoc> of the plugin can be fetched with
<Javadoc name={"org.bukkit.plugin.java.JavaPlugin#getConfig()"}>`JavaPlugin#getConfig()`</Javadoc> once it has been saved.
This will allow data to be fetched and set with the respective `#get...(key)` and
<Javadoc name={"org.bukkit.configuration.ConfigurationSection#set(java.lang.String,java.lang.Object)"}>`#set(key, value)`</Javadoc>.
By default, most basic data types are supported by YAML. These can be fetched simply with
<Javadoc name={"org.bukkit.configuration.ConfigurationSection#getString(java.lang.String)"}>`#getString(key)`</Javadoc> or
<Javadoc name={"org.bukkit.configuration.ConfigurationSection#getBoolean(java.lang.String)"}>`#getBoolean(key)`</Javadoc>.
...
<Javadoc name={"org.bukkit.Location"}>`Location`</Javadoc> and <Javadoc name={"org.bukkit.util.Vector"}>`Vector`</Javadoc>s.
...
This is possible as they implement <Javadoc name={"org.bukkit.configuration.serialization.ConfigurationSerializable"}>`ConfigurationSerializable`</Javadoc>.
...
method that takes a <Javadoc name={"java.util.Map"} project={"java"}>`Map`</Javadoc> as a parameter and returns an instance
...
If you do not call <Javadoc name={"org.bukkit.configuration.serialization.ConfigurationSerialization#registerClass(java.lang.Class)"}>`ConfigurationSerialization#registerClass(Class)`</Javadoc>
