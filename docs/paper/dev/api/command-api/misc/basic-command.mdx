...
# Basic Commands
For very simple commands Paper has a way to declare Bukkit-style commands by implementing the `BasicCommand` interface.
...
With this method, you can set up a basic `requires` structure from Brigadier commands. [You can read more on that here](../basics/command-requirements.mdx).
