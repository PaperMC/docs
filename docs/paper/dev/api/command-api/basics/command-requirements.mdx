...
How can we solve this? The `Player` interface has a method called <Javadoc name='org.bukkit.entity.Player#updateCommands()'>`#updateCommands()`</Javadoc> just for this usecase. It resends the currently registered commands back to the
