...
## Adding Data
To store data in the PDC, there are a few things you need first. The first is a <VersionedJavaDocLink target={"/org/bukkit/NamespacedKey.html"}>`NamespacedKey`</VersionedJavaDocLink> which is used to identify the data.
The second is a <VersionedJavaDocLink target={"/org/bukkit/persistence/PersistentDataContainer.html"}>`PersistentDataContainer`</VersionedJavaDocLink> which is the object you want to store the data on. The third is the data itself.
...
- A <VersionedJavaDocLink target={"/org/bukkit/plugin/Plugin.html"}>`Plugin`</VersionedJavaDocLink> instance and a `String` identifier
...
## Getting Data
To get data from the PDC, you need to know the `NamespacedKey` and the <VersionedJavaDocLink target={"/org/bukkit/persistence/PersistentDataType.html"}>`PersistentDataType`</VersionedJavaDocLink> of the data.
