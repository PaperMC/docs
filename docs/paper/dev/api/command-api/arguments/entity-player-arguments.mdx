...
to call `List#getFirst()` to retrieve that entity. 
...
which returns a Bukkit `Player` object.
...
weird behavior also explains why the `/whitelist add` command fails, so now you know 🚀.
