...
Registries are available via the <Javadoc name={"io.papermc.paper.registry.RegistryAccess"}>RegistryAccess</Javadoc>.
...
This enables the server, and to that extend plugins, to define custom content for both itself and
...
As such, this section is only applicable to [Paper plugins](./../getting-started/paper-plugins).
:::
...
Modification of a registries can take two different forms.
    ...
    public void bootstrap(@NotNull BootstrapContext context) {
        ...
        context.getLifecycleManager().registerEventHandler(RegistryEvents.ENCHANTMENT.freeze().newHandler(event -> {
            event.registry().register(
                ...
                TypedKey.create(RegistryKey.ENCHANTMENT, Key.key("papermc:pointy")), // Plugins should use their own namespace instead of minecraft or papermc
