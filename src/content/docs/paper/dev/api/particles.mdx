...
import critParticles from "./assets/particles/random-direction-crit.webp";
import smokeGoingUp from "./assets/particles/smoke-going-up.webp";
import coloredSpellParticles from "./assets/particles/orange-spell-particles.webp";
import dustParticlesInLine from "./assets/particles/blue-dust-particles-in-line.webp";
import dustTransitionParticles from "./assets/particles/dust-transition-particles.webp";
import noteParticle from "./assets/particles/note-particle.webp";
import trailParticles from "./assets/particles/trail-particles.webp";
...
<br />
<img src={critParticles.src} width={critParticles.width} height={critParticles.height} alt="Randomly directed crit particles" />
...
<br />
<img src={smokeGoingUp.src} width={smokeGoingUp.width} height={smokeGoingUp.height} alt="Campfire signal smoke going up" />
...
<br />
<img src={coloredSpellParticles.src} width={coloredSpellParticles.width} height={coloredSpellParticles.height} alt="Colored potion effect particles" />
...
:::caution
The `EFFECT` and `INSTANT_EFFECT` particles can not be colored with the API and will always be white. These particles
can only be colored by thrown potions.
:::
...
An example of creating a vertical line of green dust particles, that are 2 times the regular size:
<Tabs syncKey="spawn-type">
  <TabItem label="ParticleBuilder">
    ```java
...
<br />
<img src={dustParticlesInLine.src} width={dustParticlesInLine.width} height={dustParticlesInLine.height} alt="Blue dust particles in a vertical line" />

:::note
Here, adding a size argument would control the dust particle's lifetime, in ticks. By default, the value is a random
integer between 8 to 40, which is multiplied by the particle's scale, to get the final lifetime (with a minimum of 1).
:::
...
<br />
<img src={dustTransitionParticles.src} width={dustTransitionParticles.width} height={dustTransitionParticles.height} alt="Dust transition particles transitioning from red to blue" />
...
<br />
<img src={noteParticle.src} width={noteParticle.width} height={noteParticle.height} alt="Note particle with a light blue color" />
...
An example where 8 randomly offset trail particles travel towards a specified location
...
<br />
<img src={trailParticles.src} width={trailParticles.width} height={trailParticles.height} alt="Yellow trail particles floating towards the right side of the screen" />
...
:::note
These particles have "reverse" gravity, meaning that the initial velocity will be used only briefly, and the particle will
start to rise after a short time. Therefore, negative vertical velocity will only stop the particle from rising temporarily,
while a positive vertical velocity will make the particle rise immediately.
:::

### List of vertical velocity particles
<details>
  <summary>Show list</summary>
  - EFFECT
  - ENTITY_EFFECT
...
An example where a sweep attack particle will spawn at `someLocation` with a scale of `2.0`:
<Tabs syncKey="spawn-type">
  <TabItem label="ParticleBuilder">
    ```java
  ...
  <TabItem label="spawnParticle">
    ```java
    someWorld.spawnParticle(Particle.SWEEP_ATTACK, someLocation, 0, -2.0, 0, 0);
    ```
  </TabItem>
</Tabs>
...
An example where an explosion particle will spawn at `someLocation` with a scale of `4.0`:
<Tabs syncKey="spawn-type">
  <TabItem label="ParticleBuilder">
    ```java
  ...
  <TabItem label="spawnParticle">
    ```java
    someWorld.spawnParticle(Particle.EXPLOSION, someLocation, 0, -2.0, 0, 0);
    ```
  </TabItem>
</Tabs>
...
### Cloud particles
The `CLOUD` and `SNEEZE` particles move towards the player's y level, if they are within 2 block distance from the player's
location. When they reach the player's y level, their vertical velocity will be greatly reduced.

If the player is moving vertically, the particles will attempt to match they player's vertical velocity.
